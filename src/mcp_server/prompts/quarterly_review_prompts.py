...
class QuarterlyReviewPromptHandler(BasePromptHandler):
    ...
    async def _generate_quarterly_cycle_analysis(
...
Focus on cycle-specific insights and recommendations for sustainable quarterly delivery."""
    ...
    async def _generate_quarterly_retrospective_data(
        self, args: dict[str, Any]
    ) -> GetPromptResult:
        ...
        def _collect_retrospective_data(
            project_key: str, period_info: dict
        ) -> dict[str, Any]:
            data = {
                "period_info": period_info,
                "timestamp": self.get_current_timestamp(),
...
Identify patterns and areas for improvement within the quarterly/cycle context."""
    ...
    async def _generate_cycle_planning_insights(
        self, args: dict[str, Any]
    ) -> GetPromptResult:
        ...
        def _collect_planning_data(
            project_key: str, cycles_history: int
        ) -> dict[str, Any]:
            data = {
            ...
            try:
                ...
                time_period = (
                    f"last-{cycles_history * 3}-months"  # Assuming 3-month cycles
                )
                ...
                data["error"] = str(e)
                data["note"] = (
                    "Some historical data may be unavailable for planning analysis"
                )
