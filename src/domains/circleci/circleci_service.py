...
class CircleCIService:
        ...
        try:
            ...
            for org in collaborations:
                ...
                if pipeline_response.status_code == 200:
                    ...
                    self.logger.warning(f"Failed to get pipelines for {org_slug}. Status: {pipeline_response.status_code}")
    ...
    def generate_analysis(self, pipelines: List[Dict], workflows: List[Dict], jobs: List[Dict]) -> Dict:
        ...
        avg_duration = sum(w['duration_seconds'] for w in valid_workflows) / len(valid_workflows) if valid_workflows else 0
    ...
    def create_visualizations(self, workflows: List[Dict], jobs: List[Dict], output_path: str) -> bool:
        ...
        try:
            ...
            if not df_jobs.empty:
                ...
                # Success rate by job
                success_rates = df_jobs.groupby('name').apply(
    ...
    def save_data(self, output_dir: str, pipelines: List[Dict], workflows: List[Dict], 
                  jobs: List[Dict], analysis: Dict, recommendations: List[Dict]) -> None:
