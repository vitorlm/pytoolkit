...
class AdvancedNFCeSimilarityCommand(BaseCommand):
    ...
    def get_arguments(parser: ArgumentParser):
        ...
        parser.add_argument(
            "--model",
            type=str,
            choices=["bertimbau-large", "bertimbau-base", "legal-bertimbau", "multilingual"],
        ...
        parser.add_argument(
            "--save-db",
            action="store_true",
            help="Save results to local database"
        )
        parser.add_argument(
            "--output",
    ...
    def _print_advanced_summary(result: dict, model_name: str):
        ...
        if groups:
            ...
            for i, group in enumerate(groups, 1):
                ...
                if len(products) >= 2:
                    prod1 = products[0]['description'][:40] + "..." if len(products[0]['description']) > 40 else products[0]['description']
                    prod2 = products[1]['description'][:40] + "..." if len(products[1]['description']) > 40 else products[1]['description']
                    
                    print(f"{i}. Score: {group['similarity_score']:.3f} | Confiança: {group.get('confidence_score', 0):.3f}")
                    print(f"   {prod1}")
                    print(f"   {prod2}")
