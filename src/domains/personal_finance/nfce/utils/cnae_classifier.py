    ...
    def get_establishment_info(self, cnpj: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        self.logger.warning(f"API request failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s: {e}")
            ...
            result = {
                ...
                'cnae_principal': data.get('atividade_principal', [{}])[0].get('code') if data.get('atividade_principal') else None,
                'cnae_description': data.get('atividade_principal', [{}])[0].get('text') if data.get('atividade_principal') else None,
