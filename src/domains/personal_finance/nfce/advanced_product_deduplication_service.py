...
class AdvancedProductDeduplicationService:
    ...
    def _determine_category(self, description: str) -> str:
        ...
        category_keywords = {
            "fruits": ["ABACATE", "ABACAXI", "BANANA", "LARANJA", "LIMAO", "MEXERICA", "UVA", "MACA"],
    ...
    def _calculate_comprehensive_stats(
        ...
        # Quality metrics
        avg_quality_score = sum(p["quality_score"] for p in master_products) / len(master_products) if master_products else 0
        avg_confidence = sum(p["consolidation_confidence"] for p in master_products) / len(master_products) if master_products else 0
        
        # Establishment coverage
        establishments_covered = len(set(p["establishment_cnpj"] for p in original_products))
        avg_establishments_per_product = sum(p["unique_establishments"] for p in master_products) / len(master_products) if master_products else 0
        ...
        return {
            ...
            "quality_metrics": {
                ...
                "low_confidence_consolidations": len([p for p in master_products if p["consolidation_confidence"] < 0.7])
            },
    ...
    def _generate_cleanup_recommendations(self, statistics: Dict[str, Any]) -> List[str]:
        ...
        if reduction_pct > 50:
            ...
            recommendations.append(f"Limited deduplication: {reduction_pct:.1f}% reduction. Consider reviewing similarity thresholds")
        ...
        if quality_metrics["low_confidence_consolidations"] > 0:
            recommendations.append(f"Review {quality_metrics['low_confidence_consolidations']} low-confidence consolidations manually")
        ...
        if uncategorized_pct > 50:
            recommendations.append(f"Consider expanding category classification - {uncategorized_pct:.1f}% products uncategorized")
