...
OPTIONS:
  ...
  --export-pairs  Export test pairs for manual validation
    ...
    def get_arguments(parser: ArgumentParser):
        ...
        parser.add_argument("--samples", type=int, default=100, help="Number of product pairs to test")

        parser.add_argument("--cnpj", help="Filter by establishment CNPJ")

        parser.add_argument("--range", default="0.5-0.9", help="Threshold range for testing (format: min-max)")

        parser.add_argument("--output", help="Output file for detailed reports")

        parser.add_argument("--detailed", action="store_true", help="Show detailed analysis")

        parser.add_argument("--export-pairs", action="store_true", help="Export test pairs for manual validation")
        ...
        try:
            ...
            if args.mode == "algorithms":
                handler.compare_algorithms(
                    samples=args.samples, cnpj_filter=args.cnpj, detailed=args.detailed, export_pairs=args.export_pairs
                )

            elif args.mode == "models":
                handler.compare_embedding_models(samples=args.samples, cnpj_filter=args.cnpj, detailed=args.detailed)

            elif args.mode == "thresholds":
                threshold_range = [float(x) for x in args.range.split("-")]
                handler.optimize_thresholds(
                    samples=args.samples, cnpj_filter=args.cnpj, threshold_range=threshold_range
                )

            elif args.mode == "benchmark":
                handler.performance_benchmark(samples=args.samples, cnpj_filter=args.cnpj)

            elif args.mode == "analysis":
                handler.detailed_analysis(samples=args.samples, cnpj_filter=args.cnpj, output_file=args.output)
...
class SimilarityComparisonHandler:
    ...
    def __init__(self, logger):
        ...
        if ADVANCED_AVAILABLE:
            ...
            self.logger.warning(
                "Advanced similarity components not available - install dependencies for full functionality"
            )

    def compare_algorithms(
        self, samples: int = 100, cnpj_filter: str | None = None, detailed: bool = False, export_pairs: bool = False
    ):
    ...
    def compare_embedding_models(self, samples: int = 100, cnpj_filter: str | None = None, detailed: bool = False):
        """Compare different embedding models"""
        print("\n" + "=" * 70)
            ...
            try:
                ...
                for product1, product2 in test_pairs:
                    try:
                        ...
                        similarity = float(np.dot(emb_result1.ensemble_embedding, emb_result2.ensemble_embedding))
    ...
    def optimize_thresholds(
        self, samples: int = 100, cnpj_filter: str | None = None, threshold_range: list[float] = [0.5, 0.9]
    ):
    ...
    def performance_benchmark(self, samples: int = 500, cnpj_filter: str | None = None):
    ...
    def detailed_analysis(self, samples: int = 100, cnpj_filter: str | None = None, output_file: str | None = None):
    ...
    def _get_test_pairs(self, max_pairs: int, cnpj_filter: str | None = None) -> list[tuple[str, str]]:
    ...
    def _optimize_with_heuristics(self, samples: int, cnpj_filter: str | None, threshold_range: list[float]):
    ...
    def _export_test_pairs(self, test_pairs: list, results: dict):
        ...
        for i, (product1, product2) in enumerate(test_pairs):
            pair_data = {
                ...
                "product1": product1,
                "product2": product2,
                "algorithm_scores": {algo_name: result["scores"][i] for algo_name, result in results.items()},
            }
