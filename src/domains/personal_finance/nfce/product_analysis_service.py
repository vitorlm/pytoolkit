...
class ProductAnalysisService:
    ...
    def analyze_products_with_similarity_engine(
        ...
        if detailed:
            analysis_results["detailed_analysis"] = self._add_detailed_similarity_analysis(
        ...
        self.logger.info(f"Similarity analysis completed. Found {len(matching_results.duplicate_groups)} duplicate groups")
        return analysis_results
    ...
    def _filter_by_category(self, products: List[Dict], categories: List[str]) -> List[Dict]:
        ...
        category_keywords = {
            'beverages': ['coca', 'refri', 'suco', 'agua', 'cerveja', 'vinho', 'cafe', 'cha', 'guarana', 'sprite', 'fanta', 'pepsi', 'energetico', 'isotonic', 'achocolatado', 'leite', 'vitamina'],
            'fruits': ['banana', 'maca', 'laranja', 'uva', 'abacaxi', 'manga', 'fruta', 'limao', 'melancia', 'melao', 'mamao', 'goiaba', 'pera', 'pessego', 'morango', 'kiwi', 'mexerica', 'tangerina', 'abacate', 'caju', 'acerola'],
            'dairy': ['leite', 'queijo', 'iogurte', 'manteiga', 'nata', 'creme', 'requeijao', 'ricota', 'mussarela', 'parmesao', 'gouda', 'coalho'],
            'meat': ['carne', 'frango', 'peixe', 'porco', 'boi', 'linguica', 'salsicha', 'hamburguer', 'bacon', 'presunto', 'mortadela', 'salame', 'peito', 'coxa', 'asa', 'file', 'costela', 'alcatra', 'picanha'],
            'bread': ['pao', 'biscoito', 'bolo', 'torrada', 'padaria', 'croissant', 'brioche', 'rosca', 'sonho', 'bolacha', 'wafer', 'cracker'],
            'cleaning': ['detergente', 'sabao', 'amaciante', 'desinfetante', 'limpa', 'alvejante', 'multiuso', 'vidro', 'chao', 'vaso'],
            'snacks': ['salgadinho', 'chips', 'pipoca', 'amendoim', 'castanha', 'chocolate', 'bala', 'chiclete', 'pirulito', 'doce', 'guloseima'],
            'grains': ['arroz', 'feijao', 'macarrao', 'massa', 'farinha', 'aveia', 'quinoa', 'granola', 'cereal', 'milho', 'trigo'],
            'condiments': ['sal', 'acucar', 'oleo', 'vinagre', 'tempero', 'molho', 'ketchup', 'mostarda', 'maionese', 'pimenta', 'oregano', 'cominho'],
            'frozen': ['congelado', 'sorvete', 'picole', 'lasanha', 'nuggets', 'hamburguer', 'pizza', 'gelado'],
            'personal_care': ['shampoo', 'condicionador', 'sabonete', 'pasta', 'dente', 'desodorante', 'perfume', 'creme', 'hidratante'],
            'vegetables': ['tomate', 'cebola', 'alho', 'batata', 'cenoura', 'abobrinha', 'pimentao', 'alface', 'couve', 'brocolis', 'pepino', 'verdura', 'legume']
        }
    ...
    def _analyze_basic_statistics(self, products: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "duplication_ratio": round((total_products - unique_descriptions) / total_products * 100, 2) if total_products > 0 else 0,
            "average_occurrence_per_product": round(avg_occurrence, 2),
            "most_common_units": dict(unit_counts.most_common(10)),
            "establishments_per_product": round(total_products / unique_descriptions, 2) if unique_descriptions > 0 else 0
        }
    ...
    def _analyze_by_establishment(self, products: List[Dict]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'avg_products_per_establishment': round(sum(establishment_counts) / len(establishment_counts), 2) if establishment_counts else 0,
            'avg_unique_products_per_establishment': round(sum(unique_counts) / len(unique_counts), 2) if unique_counts else 0,
    ...
    def _analyze_naming_patterns(self, products: List[Dict], min_frequency: int) -> Dict[str, Any]:
        ...
        for word, count in word_freq.most_common(50):
            if len(word) > 2 and count >= min_frequency * 2:
                ...
                first_word_count = sum(1 for desc in descriptions 
                                     if desc.upper().startswith(word))
    ...
    def _find_potential_duplicates(
        ...
        return {
            ...
            'summary': {
                ...
                'largest_duplicate_group': max([g['product_count'] for g in duplicate_groups]) if duplicate_groups else 0,
                'avg_establishments_per_group': round(
                    sum(g['establishment_count'] for g in duplicate_groups) / len(duplicate_groups), 2
    ...
    def _analyze_categories(self, products: List[Dict]) -> Dict[str, Any]:
        ...
        categories = {
            'beverages': ['coca', 'refri', 'suco', 'agua', 'cerveja', 'cafe', 'cha', 'guarana', 'sprite', 'fanta', 'pepsi', 'energetico', 'isotonic', 'achocolatado', 'leite', 'vitamina'],
            'fruits': ['banana', 'maca', 'laranja', 'uva', 'fruta', 'abacaxi', 'limao', 'melancia', 'melao', 'mamao', 'goiaba', 'pera', 'pessego', 'morango', 'kiwi', 'mexerica', 'tangerina', 'abacate', 'caju', 'acerola'],
            'dairy': ['leite', 'queijo', 'iogurte', 'manteiga', 'nata', 'requeijao', 'ricota', 'mussarela', 'parmesao', 'gouda', 'coalho'],
            'meat': ['carne', 'frango', 'peixe', 'porco', 'boi', 'linguica', 'salsicha', 'hamburguer', 'bacon', 'presunto', 'mortadela', 'salame', 'peito', 'coxa', 'asa', 'file', 'costela', 'alcatra', 'picanha'],
            'bread': ['pao', 'biscoito', 'bolo', 'torrada', 'padaria', 'croissant', 'brioche', 'rosca', 'sonho', 'bolacha', 'wafer', 'cracker'],
            'cleaning': ['detergente', 'sabao', 'amaciante', 'limpa', 'alvejante', 'multiuso', 'vidro', 'chao', 'vaso'],
            'snacks': ['salgadinho', 'chips', 'pipoca', 'amendoim', 'castanha', 'chocolate', 'bala', 'chiclete', 'pirulito', 'doce', 'guloseima'],
            'grains': ['arroz', 'feijao', 'macarrao', 'massa', 'farinha', 'aveia', 'quinoa', 'granola', 'cereal', 'milho', 'trigo'],
            'condiments': ['sal', 'acucar', 'oleo', 'vinagre', 'tempero', 'molho', 'ketchup', 'mostarda', 'maionese', 'pimenta', 'oregano', 'cominho'],
            'frozen': ['congelado', 'sorvete', 'picole', 'lasanha', 'nuggets', 'hamburguer', 'pizza', 'gelado'],
        ...
        return {
            ...
            'categorization_rate': round(
                ...
                sum(category_counts.values()) * 100, 2
            ) if category_counts else 0,
    ...
    def print_analysis_summary(self, results: Dict[str, Any]) -> None:
        ...
        if groups:
            ...
            for i, group in enumerate(groups[:5], 1):
                print(f"  {i}. '{group['normalized_description']}' - {group['product_count']} products across {group['establishment_count']} establishments")
    ...
    def _add_establishment_context_to_duplicates(self, products: List[Dict], matching_results) -> Dict[str, Any]:
        ...
        establishments_with_duplicates = sum(
            1 for _, data in establishment_summary.items() 
    ...
    def print_similarity_analysis_summary(self, results: Dict[str, Any]) -> None:
        ...
        if duplicate_groups:
            ...
            for i, group in enumerate(duplicate_groups[:5], 1):
                print(f"  {i}. '{group['representative_product']}' - {group['size']} products (similarity: {group['avg_similarity']:.3f})")
        ...
        if ranking:
            ...
            for i, est in enumerate(ranking[:3], 1):
                print(f"  {i}. {est['business_name']} - {est['duplicate_products']}/{est['total_products']} products ({est['duplicate_ratio']:.1%})")
    ...
    def analyze_similarity_only(
        ...
        self.logger.info(f"Similarity analysis complete: {len(similarity_results.duplicate_groups)} duplicate groups found")
