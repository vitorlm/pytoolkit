...
class SimilarityCalculator:
    ...
    def __init__(self, similarity_threshold: float = 0.60):
        ...
        self.penalties = {
            'different_category': -0.3,
            'no_token_overlap': -0.2
        }
    ...
    def calculate_similarity(self, features1: ProductFeatures, features2: ProductFeatures) -> SimilarityResult:
        ...
        self.logger.debug(f"Calculating similarity between '{features1.original_description}' and '{features2.original_description}'")
        ...
        final_score = self._calculate_final_score(
            jaccard_score, cosine_score, levenshtein_score, token_overlap_score,
            features1, features2, matching_tokens
        )
    ...
    def _calculate_final_score(self, jaccard: float, cosine: float, levenshtein: float, 
                              token_overlap: float, features1: ProductFeatures, 
                              features2: ProductFeatures, matching_tokens: List[str]) -> float:
        """
    ...
    def find_duplicates(self, features_list: List[ProductFeatures], 
                       duplicate_threshold: float = None) -> List[SimilarityResult]:
        """
        ...
        for result in duplicates:
            ...
            if (result.final_score >= duplicate_threshold and
                result.category_match and
                len(result.matching_tokens) >= 2):
