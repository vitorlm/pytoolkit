    ...
    def store_invoice_data(self, invoice_data: InvoiceData) -> bool:
        ...
        try:
            ...
            try:
                ...
                conn.rollback()
                self.logger.error(
                    f"Error storing invoice {invoice_data.access_key}: {e}",
                    exc_info=True,
                )
    ...
    def _store_establishment(self, conn, establishment: EstablishmentData) -> str | None:
        ...
        try:
            ...
            if not existing or not existing[1]:  # New establishment or missing type
                ...
                if establishment.cnpj in self._cnae_session_cache:
                    ...
                    if not cached_root_data:
                        ...
                        if db_related:
                            self.logger.info(
                                f"Using related company database data for CNPJ: {establishment.cnpj} (CNAE: {db_related[0]})"
                            )
                ...
                if cnae_data:
                    ...
                    self.logger.warning(
                        f"Could not classify establishment type for CNPJ: {establishment.cnpj} - API returned None"
                    )
            ...
            if existing:
                ...
                self.logger.info(
                    f"Stored new establishment: {establishment.business_name} ({establishment_type}) - {'MATRIZ' if is_main_office else 'FILIAL'}"
                )
    ...
    def _store_invoice(self, conn, invoice_data: InvoiceData, establishment_id: str) -> str:
        ...
        try:
            ...
            establishment_cnpj = conn.execute(
                "SELECT cnpj FROM establishments WHERE id = ?", [establishment_id]
            ).fetchone()[0]
