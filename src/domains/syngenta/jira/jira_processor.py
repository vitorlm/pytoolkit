...
class JiraProcessor:
    ...
    def _analyze_changelog(self, changelog: list[dict]) -> tuple[date | None, date | None]:
        ...
        for history in changelog:
            for item in history.get("items", []):
                if item.get("toString") == "7 PI Started" and not start_date:
                    start_date = datetime.strptime(history["created"], "%Y-%m-%dT%H:%M:%S.%f%z").date()
                if item.get("fromString") == "7 PI Started" and item.get("toString") == "Done" and not end_date:
                    end_date = datetime.strptime(history["created"], "%Y-%m-%dT%H:%M:%S.%f%z").date()
                if start_date and end_date:
                    break
        ...
        try:
            ...
            for epic in epics:
                ...
                if (
                    ...
                    and (not epic["fields"]["customfield_10015"] or not epic["fields"]["customfield_10233"])
                ):
