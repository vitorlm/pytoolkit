...
class IssueVelocityService:
    ...
    def analyze_issue_velocity(
        ...
        try:
            ...
            self.logger.info(f"Total: {len(created_issues)} created, {len(resolved_issues)} resolved issues")

            # Group issues by time periods
            periods = self._get_time_periods(start_date, end_date, aggregation)

            # Calculate velocity metrics
            velocity_data = self._calculate_velocity_metrics(created_issues, resolved_issues, periods, issue_types)
            ...
            result = {
                ...
                "by_issue_type": self._calculate_breakdown_by_type(created_issues, resolved_issues, issue_types),
            }
    ...
    def _parse_time_period(self, time_period: str) -> tuple[datetime, datetime]:
        ...
        try:
            ...
            raise ValueError(f"Invalid time period format: {time_period}. {e!s}")
    ...
    def _get_time_periods(self, start_date: datetime, end_date: datetime, aggregation: str) -> list[dict]:
        ...
        while current_date <= end_date:
            if aggregation == "monthly":
                ...
                if quarter_end_month == 12:
                    quarter_end = current_date.replace(month=12, day=31, hour=23, minute=59, second=59)
                else:
    ...
    def _calculate_velocity_metrics(
        ...
        for period_info in periods:
            ...
            created_count = self._count_issues_in_period(created_issues, period_start, period_end, "created")

            # Count resolved issues in this period
            resolved_count = self._count_issues_in_period(resolved_issues, period_start, period_end, "resolved")
    ...
    def _calculate_summary_statistics(self, velocity_data: list[dict], _aggregation: str) -> dict:
        ...
        if len(velocity_data) >= 2:
            first_half_avg = sum(p["efficiency_percentage"] for p in velocity_data[: len(velocity_data) // 2]) / (
                len(velocity_data) // 2
            )
            second_half_avg = sum(p["efficiency_percentage"] for p in velocity_data[len(velocity_data) // 2 :]) / (
