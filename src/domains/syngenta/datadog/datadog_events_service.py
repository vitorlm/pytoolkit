...
class DatadogEventsService:
        ...
        *,
        ...
        for event in events or []:
            ...
            normalized.append(
                {
                    "id": event.get("id"),
                    "title": attributes.get("title") or attributes.get("text") or event_attributes.get("title") or "",
                    ...
                    "duration_seconds": self._nanoseconds_to_seconds(event_attributes.get("duration")),
    ...
    @staticmethod
    def _next_cursor(response_json: dict[str, Any]) -> str | None:
    ...
    def _safe_monitor_payload(monitor: Any) -> dict[str, Any]:
        ...
        alert_cycle_key = (
            monitor.get("alert_cycle_key") or monitor.get("alert_cycle_key_txt") or monitor.get("alert_cycle")
        )
    ...
    @staticmethod
    def _ensure_epoch_timestamp(value: Any) -> float | None:
        if value is None:
            return None
        if isinstance(value, (int, float)):
            try:
    ...
    @staticmethod
    def _extract_team_from_tags(tags: Any) -> str | None:
    ...
    @staticmethod
    def _extract_env_from_tags(tags: Any) -> str | None:
        if not isinstance(tags, list):
            return None
        for tag in tags:
    ...
    @staticmethod
    def _extract_latest_timestamp(events: list[dict[str, Any]]) -> str | None:
    ...
    def find_unused_monitors(
        ...
        *,
        ...
        cutoff_timestamp = datetime.now(UTC) - timedelta(days=days)
        ...
        for monitor in all_monitors:
            ...
            if last_triggered_ts and not is_active:
                try:
                    last_triggered = datetime.fromtimestamp(last_triggered_ts, tz=UTC)
            ...
            if last_triggered_ts:
                try:
                    last_triggered = datetime.fromtimestamp(last_triggered_ts, tz=UTC)
                    days_diff = (datetime.now(UTC) - last_triggered).days
