...
class IssueSummary(BaseModel):
    ...
    planned_start_date: date | None = Field(None, description="Planned start date.")
    actual_start_date: date | None = Field(None, description="Actual start date.")
    planned_end_date: date | None = Field(None, description="Planned end date.")
    actual_end_date: date | None = Field(None, description="Actual end date.")

    planned_issue_days: int | None = Field(None, description="Planned duration in days.")
    executed_issue_days: int | None = Field(None, description="Actual duration in days.")

    planned_resources: int | None = Field(None, description="Planned number of resources.")
    actual_resources: int | None = Field(None, description="Actual number of resources.")

    executed: bool | None = Field(None, description="Indicates whether the issue was executed.")
    closed: bool | None = Field(None, description="Indicates whether the issue is closed.")
    ...
    def schedule_adherence(self) -> float:
        ...
        if self.planned_issue_days:
            return (self.executed_issue_days / self.planned_issue_days) * 100 if self.executed_issue_days else 0
        return 0.0
    ...
    @property
    def date_deviation(self, unit: str = "days") -> int | float:
    ...
    def duration_deviation_percentage(self) -> float:
        ...
        if self.planned_issue_days:
            return ((self.executed_issue_days - self.planned_issue_days) / self.planned_issue_days) * 100
        return 0.0
    ...
    def execution_ratio(self) -> float:
        ...
        if self.planned_issue_days:
            return self.executed_issue_days / self.planned_issue_days if self.executed_issue_days else 0.0
        return 0.0
    ...
    def efficiency(self, duration_weight: float = 0.5, resource_weight: float = 0.5) -> float:
        ...
        if self.executed_issue_days and self.planned_issue_days:
            duration_efficiency = min((self.planned_issue_days / self.executed_issue_days) * 100, 100)  # Cap at 100%

        if self.actual_resources and self.planned_resources:
            resource_efficiency = min((self.planned_resources / self.actual_resources) * 100, 100)  # Cap at 100%

        return min(
            (duration_efficiency * duration_weight) + (resource_efficiency * resource_weight),
            100,
        )

    @property
    def resource_utilization(self) -> float | None:
    ...
    @property
    def resource_allocation_accuracy(self) -> float | None:
    ...
    @property
    def resource_variance(self) -> int | None:
    ...
    @property
    def resource_efficiency_index(self) -> float | None:
...
class IssueDetails(BaseModel):
    ...
    start_date: date | None = Field(None, description="Start date of the issue.")
    end_date: date | None = Field(None, description="End date of the issue.")
    issue_total_days: int | None = Field(0, description="Total days allocated for the issue.")
    member_list: list[str] = Field(default_factory=list, description="List of team members assigned to the issue.")
...
class Issue(BaseModel):
    ...
    category: str | None = Field(
        None,
        description="Activity category: DEVELOPMENT, SUPPORT, IMPROVEMENT, OVERDUE, SECONDED, LEARNING, PLANNING, ABSENCE",
    )
    priority: str | int | None = Field(None, description="Priority/Rank of the issue (R#).")
    dev_type: str | None = Field(None, description="Development type (BE/FE).")
    jira: str | None = Field(None, description="Jira ID associated with the issue.")
    description: str | None = Field(None, description="Description of the issue.")

    closed: bool | None = Field(False, description="Indicates whether the issue is closed.")

    planned: IssueDetails | None = Field(None, description="Planned details for the issue.")
    executed: IssueDetails | None = Field(None, description="Executed details for the issue.")

    summary: IssueSummary | None = Field(None, description="Summary of planned and actual metrics.")
