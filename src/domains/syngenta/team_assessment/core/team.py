...
class TeamSummary(BaseModel):
    ...
    members_contribution: Dict[str, Dict[str, int]] = Field(
        default_factory=dict, description="Contributions of each member across all cycles."
    )
...
class Team(BaseModel):
    ...
    start_date: Optional[date] = Field(None, description="Start date of the earliest cycle.")
    end_date: Optional[date] = Field(None, description="End date of the latest cycle.")
    summary: Optional[TeamSummary] = Field(None, description="Summary of the team's performance.")
    ...
    def summarize(self) -> None:
        ...
        epics_completion_rate = round(
            ((total_closed_epics / total_epics) * 100 if total_epics or total_epics == 0 else 0), 2
        )
        ...
        bugs_per_day = (
            round(total_bugs / net_available_work_days, 2) if net_available_work_days else 0
        )
