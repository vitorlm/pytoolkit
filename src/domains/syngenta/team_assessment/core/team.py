...
class TeamSummary(BaseModel):
    ...
    total_non_epics: int = Field(
        0, description="Total number of non-epics across all cycles."
    )
    total_closed_epics: int = Field(
        0, description="Total number of closed epics across all cycles."
    )
    ...
    total_spillover_days: int = Field(
        0, description="Total spillover days across all cycles."
    )
    total_off_days: int = Field(0, description="Total off days across all cycles.")
    total_executed_days: int = Field(
        0, description="Total executed days across all cycles."
    )
    total_planned_days: int = Field(
        0, description="Total planned days across all cycles."
    )
    epics_completion_rate: float = Field(
        0.0, description="Epics completion rate across all cycles."
    )
    average_adherence_to_dates: float = Field(
        0.0, description="Average adherence to dates across all cycles."
    )
    ...
    average_efficiency: float = Field(
        0.0, description="Average efficiency across all cycles."
    )
    average_resource_utilization: float = Field(
        0.0, description="Average resource utilization across all cycles."
    )
    ...
    average_bug_days_per_cycle: float = Field(
        0.0, description="Average bug days per cycle."
    )
    average_bugs_per_cycle: float = Field(0.0, description="Average bugs per cycle.")
    average_spillover_days_per_cycle: float = Field(
        0.0, description="Average spillover days per cycle."
    )
    average_off_days_per_cycle: float = Field(
        0.0, description="Average off days per cycle."
    )
    efficiency: float = Field(0.0, description="Efficiency across all cycles.")
    bug_workload_percentage: float = Field(
        0.0, description="Bug workload percentage across all cycles."
    )
    bugs_epic_ratio: float = Field(
        0.0, description="Bugs epic ratio across all cycles."
    )
    spillover_workload_percentage: float = Field(
        0.0, description="Spillover workload percentage across all cycles."
    )
    net_available_work_days: int = Field(
        0, description="Net available work days across all cycles."
    )
    effective_work_days: int = Field(
        0, description="Effective work days across all cycles."
    )
    days_off_impact_ratio: float = Field(
        0.0, description="Days off impact ratio across all cycles."
    )
...
class Team(BaseModel):
    ...
    cycles: Dict[str, Cycle] = Field(
        default_factory=dict, description="Cycles data for the team."
    )
    start_date: Optional[date] = Field(
        None, description="Start date of the earliest cycle."
    )
    end_date: Optional[date] = Field(None, description="End date of the latest cycle.")
    summary: Optional[TeamSummary] = Field(
        None, description="Summary of the team's performance."
    )
    ...
    def summarize(self) -> None:
        ...
        for cycle in self.cycles.values():
            ...
            for epic in cycle.backlog.values():
                if (
                    epic
                    and epic.code in self._config.issue_helper_codes
                    and epic.code not in unique_non_epics
                ):
            ...
            for member, contribution in summary.members_contribution.items():
                ...
                members_contribution[member]["planned_item"] += contribution.get(
                    "planned_item", 0
                )
        ...
        epics_completion_rate = round(
            (
                (total_closed_epics / total_epics) * 100
        ...
        average_epics_per_cycle = (
            round(total_epics / cycle_count, 2) if cycle_count else 0
        )
        average_bugs_per_cycle = (
            round(total_bugs / cycle_count, 2) if cycle_count else 0
        )
        average_bug_days_per_cycle = (
            round(total_bug_days / cycle_count, 2) if cycle_count else 0
        )
        average_spillover_days_per_cycle = (
            round(total_spillover_days / cycle_count, 2) if cycle_count else 0
        )
        average_off_days_per_cycle = (
            round(total_off_days / cycle_count, 2) if cycle_count else 0
        )

        net_available_work_days = total_work_days - total_off_days

        effective_work_days = (
            total_work_days - total_spillover_days - total_bug_days - total_off_days
        )

        efficiency = round(
            (
                (net_available_work_days / total_executed_days) * 100
                if total_executed_days
                else 0
            ),
            2,
        )
        ...
        bugs_per_day = (
            round(total_bugs / net_available_work_days, 2)
            if net_available_work_days
            else 0
        )
        ...
        days_off_impact_ratio = round(
            (
                (total_off_days / total_work_days) * 100
                if total_work_days or total_work_days == 0
                else 0
            ),
            2,
        )
        ...
        summary = {
            ...
            "average_adherence_to_dates": (
                round(average_adherence_to_dates / cycle_count, 2) if cycle_count else 0
            ),
            "adherence_categories": adherence_categories,
            "average_efficiency": round(average_efficiency / cycle_count, 2)
            if cycle_count
            else 0,
            "average_resource_utilization": (
                round(average_resource_utilization / cycle_count, 2)
                if cycle_count
                else 0
            ),
