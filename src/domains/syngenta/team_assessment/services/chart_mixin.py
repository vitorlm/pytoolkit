...
class ChartMixin:
    ...
    def plot_horizontal_bar_chart(
        ...
        if group_col is not None:
            if group_colors is None:
                ...
                group_colors = {
                    group: cmap(i % cmap.N) for i, group in enumerate(unique_groups)
                }
    ...
    def plot_grouped_bar_chart(
        ...
        ax.set_title(
            title if title else "Grouped Bar Chart", fontsize=16, weight="bold"
        )
        ax.set_xticks(x)
    ...
    def plot_radar_chart(
        ...
        for series_name, values in data.items():
            if not values:
                raise ValueError(
                    "Each series in data must have at least one set of values."
                )
        ...
        ax.set_thetagrids(
            np.degrees(angles[:-1]), short_labels, fontsize=12, weight="bold"
        )
